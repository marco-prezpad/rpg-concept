...
class Personaje {
 public:
  CommonAttributes& GetCommonAttributes() {return common_attributes_;}
...
// usar el struct de atributos generales (arreglar)
  ...
  std::string name_;
  double attack_; // Incluido en el struct
  double defense_; // Incluido en el struct
  double personal_crit_chance_;
  double total_crit_chance_; // critico general arma mas personaje
  double life_; // Incluido en el struct
  double speed_; // Inlcuido en el struct
  double exp_; // Inlcuido en el struct
  // Ver struct
